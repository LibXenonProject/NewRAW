...
void SDLStub::init(const char *title) {
	...
	_fullscreen = true;
	_scaler = 2; //MagicSeb : Better Scaler
	prepareGfxMode();

	SDL_JoystickEventState(SDL_ENABLE);
    Joystick = SDL_JoystickOpen(0);

	//SDL_XBOX_SetScreenPosition(0, 0);
	//SDL_XBOX_SetScreenStretch(-20, 0);
}
...
void SDLStub::processEvents() {
	...
	while(SDL_PollEvent(&ev)) {
		...
		case SDL_JOYBUTTONDOWN:
			if (ev.button.button == 8)
				_pi.save = true;
			else if (ev.button.button == 9)
				_pi.load = true;
			else if (ev.button.button == 4)
				_pi.fastMode = true;
			else if (ev.button.button == 5)
				_pi.code = true;
			else if (ev.button.button == 11)
				_pi.quit = true;
		...
		case SDL_JOYAXISMOTION:
			if (ev.jaxis.axis == 0) {  /* left-right */
				if (ev.jaxis.value < -SYSJOY_RANGE ) {  /* left */
				_pi.dirMask |= PlayerInput::DIR_LEFT;
				_pi.dirMask &= ~PlayerInput::DIR_RIGHT;
				}
				else if (ev.jaxis.value > SYSJOY_RANGE) {  /* right */
				_pi.dirMask |= PlayerInput::DIR_RIGHT;
				_pi.dirMask &= ~PlayerInput::DIR_LEFT;
				}
				else {  /* center */
			...
			if (ev.jaxis.axis == 1) {  /* up-down */
				if (ev.jaxis.value < -SYSJOY_RANGE) {  /* up */
				_pi.dirMask |= PlayerInput::DIR_UP;
				_pi.dirMask &= ~PlayerInput::DIR_DOWN;
				}
				else if (ev.jaxis.value > SYSJOY_RANGE) {  /* down */
				_pi.dirMask |= PlayerInput::DIR_DOWN;
				_pi.dirMask &= ~PlayerInput::DIR_UP;
				}
				else {  /* center */
